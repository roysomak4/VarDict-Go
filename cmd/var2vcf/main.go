...
func parseFlags() Config {
	...
	flag.StringVar(&config.RefPath, "G", "", "Path to the *.fasta (*.fa) file which is used to generate reference tag in the header")
	flag.StringVar(&config.BedPath, "b", "", "Path to the *.bed file which is used to generate contigs in the header")
	flag.IntVar(&config.MinTotalDepth, "d", 3, "integer\nThe minimum total depth.  Default to 3, might consider lower depth if (high quality variant count)*af >= 0.5")
	flag.IntVar(&config.MinVariantDepth, "v", 2, "integer\nThe minimum high quality variant depth.  Default to 2.  Would consider lower depth (high quality variant count)*af >= 0.5")
	flag.Float64Var(&config.MinAF, "f", 0.02, "float\nThe minimum allele fraction.  Default to 0.02")
	flag.Float64Var(&config.MinSN, "o", 1.5, "float\nThe minimum signal to noise, or the ratio of hi/(lo+0.5).  Default to 1.5, that is both 2 variant reads are high quality.")
	flag.Float64Var(&config.MinPMean, "p", 5.0, "float\nMinimum mean position in read")
	flag.Float64Var(&config.MinQual, "q", 22.5, "float\nMinimum base quality")
	flag.Float64Var(&config.MinMapQ, "Q", 20.0, "float\nMinimum mapping quality")
	flag.Float64Var(&config.GTFreq, "F", 0.2, "float\nGenotype frequency threshold")
	flag.BoolVar(&config.PrintEndTag, "E", false, "If set, do not print END tag in INFO field")
	...
	flag.IntVar(&config.MinSplitReads, "T", 1, "integer\nMinimum split reads for structural variants")
